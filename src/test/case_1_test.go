...
func TestParallelLoop_Get_From_Header_And_Path(t *testing.T) {
	projectDir := dirName + "/test-1.3"

	req, _ := http.NewRequest("POST", URL+"/test-1-3/nicholas", nil)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("x-api-key", "123123123")
	...
	assert.Equalf(t, http.StatusCreated, res.StatusCode,
		" Expected %s but got %s", http.StatusCreated, res.StatusCode)
	...
	expectedByte := util.ReadJsonFile(projectDir + "/test-1.3_expected.json")
	equal, err := util.JSONBytesEqual(expectedByte, responseByte)
	if err != nil {
		assert.Error(t, err, "error compare json byte")
	}
