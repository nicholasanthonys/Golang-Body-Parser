...
func TestSerial_To_Parallel(t *testing.T) {
	projectDir := dirName + "/test-8.2_serial_to_parallel"
	mapBody := map[string]interface{}{
		"content": "selamat anda menang",
		"phone_numbers": []string{
			"123456789",
	...
	req, _ := http.NewRequest("POST", URL+"/test-8-2", body)
	req.Header.Set("Content-Type", "application/json")
	...
	expectedByte := util.ReadJsonFile(projectDir + "/test-8.2_expected.json")
	equal, err := util.JSONBytesEqual(expectedByte, responseByte)
	if err != nil {
		assert.Error(t, err, "error compare json byte")
	}
