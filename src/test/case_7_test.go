...
func TestParallel_Logic_True_No_Next_Success(t *testing.T) {
	projectDir := dirName + "/test-7.2"
	...
	req, _ := http.NewRequest("POST", URL+"/test-7-2", body)
	req.Header.Set("Content-Type", "application/json")
	...
	expectedByte := util.ReadJsonFile(projectDir + "/test-7.2_expected.json")
	equal, err := util.JSONBytesEqual(expectedByte, responseByte)
	if err != nil {
		assert.Error(t, err, "error compare json byte")
	}
...
func TestParallel_Logic_False_Refer_To_Another_Request(t *testing.T) {
	projectDir := dirName + "/test-7.3"
	...
	req, _ := http.NewRequest("POST", URL+"/test-7-3", body)
	req.Header.Set("Content-Type", "application/json")
	...
	expectedByte := util.ReadJsonFile(projectDir + "/test-7.3_expected.json")
	equal, err := util.JSONBytesEqual(expectedByte, responseByte)
	if err != nil {
		assert.Error(t, err, "error compare json byte")
	}
...
func TestParallel_Logic_False_No_Next_Failure(t *testing.T) {
	projectDir := dirName + "/test-7.4"
	...
	req, _ := http.NewRequest("POST", URL+"/test-7-4", body)
	req.Header.Set("Content-Type", "application/json")
	...
	expectedByte := util.ReadJsonFile(projectDir + "/test-7.4_expected.json")
	equal, err := util.JSONBytesEqual(expectedByte, responseByte)
	if err != nil {
		assert.Error(t, err, "error compare json byte")
	}
